...
static void runSumEvens(struct node **t, int argc, char *argv[]);
...
static void runTrim(struct node **t, int argc, char *argv[]);
...
static Command COMMANDS[] = {
	...
	{"e", runSumEvens,          "",          "get the sum of even values in the BST"},
	...
	{"t", runTrim,              "<depth>",   "delete all nodes below the given depth"},
...
static void runTrim(struct node **t, int argc, char *argv[]) {
	...
	int depth = 0;
	char c;
	if (sscanf(argv[1], "%d%c", &depth, &c) != 1) {
		printf("Error: Invalid depth '%s'\n", argv[1]);
		return;
	}

	*t = bstTrim(*t, depth);
	printf("Deleted all nodes below depth %d\n", depth);
}
