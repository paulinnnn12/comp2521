...
bool bstFind(struct node *t, int value) {
	if (t == NULL) {
		...
		return bstFind(t->left, value);
	} else if (value > t->value) {
		return bstFind(t->right, value);
	} else { // (value == t->value)
		return true;
	}
}
...
// Returns the number of even values in the given BST
int bstSumEvens(struct node *t) {
	if (t == NULL) {
		return 0;
	}
	if (t->value % 2 == 0) {
		return t->value + bstSumEvens(t->left) + bstSumEvens(t->right); 
	} else {
			return bstSumEvens(t->left) + bstSumEvens(t->right); 
	}
}
...
int bstRange(struct node *t) {
	...
	struct node *low_node = t;
	struct node *high_node = t;
	while (low_node->left != NULL) {
	...
	return (high_node->value - low_node->value);
}
...
struct node *bstDeleteLeaves(struct node *t) {
	...
	// in case out root have no child (one side or both)
	if (t->left == NULL && t->right == NULL) {
		free(t);
		return NULL;
	}
...
int bstClosest(struct node *t, int value) {
    int closest_value = t->value;
    struct node *curr = t;

    while (curr != NULL) {
        if (abs(curr->value - value) < abs(closest_value - value)) {
...
struct node *bstTrim(struct node *t, int depth) {
	...
	t->left = bstTrim(t->left, depth - 1);
	t->right = bstTrim(t->right, depth - 1);
